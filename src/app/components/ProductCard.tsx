...
interface ProductCardProps {
  product: Product;
  countryId: string;
  onOpen?: (product: Product) => void; // gardé pour compatibilité mais plus utilisé
}
    ...
    <div className="flex gap-0.5" aria-label={`Note: ${rating} sur 5`}>
      {[1, 2, 3, 4, 5].map((i) => (
        <Star
          key={i}
          className={`w-3 h-3 ${
            i <= Math.round(rating) ? "fill-[#D2A63B] text-[#D2A63B]" : "text-gray-300"
          }`}
        />
      ))}
    </div>
...
export function ProductCard({ product, countryId }: ProductCardProps) {
  ...
  // Toujours naviguer vers la page dédiée, mobile ET PC
      ...
      <div className="relative h-44 bg-muted overflow-hidden">
        ...
        {product.taxFree && (
          <div className="absolute top-2 right-2">
            <span className="inline-flex items-center gap-1 text-xs font-semibold bg-[#1E4D3B]/10 text-[#1E4D3B] px-2 py-0.5 rounded-full">
        ...
        {supplier && (
          ...
          >
            ...
            <div className="flex items-center gap-1.5">
              ...
              {supplier.verified && (
                <ShieldCheck className="w-3 h-3 text-[#1E4D3B] inline" title={t("verified", language)} />
              )}
            </div>
          </Link>
        )}
        ...
        <div className="mt-auto pt-3 border-t border-border">
          <div className="flex items-end justify-between">
            ...
            <button
              ...
              className="bg-primary text-primary-foreground px-3 py-2 rounded-lg text-sm font-semibold hover:bg-[#163d2e] transition-colors flex items-center gap-1"
            >
              Voir plus
